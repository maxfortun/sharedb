    ...
    backend.db.commit(
      ...
      function(err, succeeded) {
        ...
        if (!request.suppressPublish) {
          ...
          op.d = request.id;
          if(this.options && this.options.opsOptions && this.options.opsOptions.metadata) {
            if(typeof op.m === 'object' && typeof options.opsOptions.metadata === 'object') {
              Object.keys(op.m).forEach(key => {
                if(!options.opsOptions.metadata[key]) {
                  delete op.m[key];
                }
              });
            }
