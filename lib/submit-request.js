    ...
    backend.db.commit(
      ...
      function(err, succeeded) {
        ...
        if (!request.suppressPublish) {
          ...
          op.d = request.id;
          if(backend.options && backend.options.opsOptions && backend.options.opsOptions.metadata) {
            if(typeof op.m === 'object' && typeof backend.options.opsOptions.metadata === 'object') {
              Object.keys(op.m).forEach(key => {
                if(!backend.options.opsOptions.metadata[key]) {
                  delete op.m[key];
                }
              });
            }
