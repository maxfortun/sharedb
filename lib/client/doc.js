...
Doc.prototype._otApply = function(op, source) {
  if ('op' in op) {
    ...
    this.emit('before op batch', op.op, source, op.src, { op });
    ...
    if (!source && this.type === types.defaultType && op.op.length > 1) {
      ...
      for (var i = 0; i < op.op.length; i++) {
        ...
        this.emit('op', componentOp.op, source, op.src, { op });
      }
      this.emit('op batch', op.op, source, { op });
    ...
    this.emit('before op', op.op, source, op.src, { op });
    ...
    this.emit('op', op.op, source, op.src, { op });
    this.emit('op batch', op.op, source, op.src, { op });
    return;
  }
